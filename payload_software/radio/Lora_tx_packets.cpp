...
std::vector<int> getPackets(std::string str, int filesize) {
    ...
    if (buf!="") {vec.push_back(stoi(buf));};

    //Exclude out of bounds packets
    std::vector<int> packets;
    for (int packetnum : vec) {
        if (packetnum*PACKET_SIZE<filesize) {packets.push_back(packetnum);}
        else {puts("Packet is out of bounds");}
    }
    return packets;
}
...
int main(int argc, char **argv) {
	if (argc < 4) {
		printf("Usage: Lora_tx_packets <freq in MHz> <file to send> <packets to send>\n");
		return 1;
	}
    ...
    printf("Sending %d packets (%d bytes)...\n", packets.size(), packets.size()*PACKET_SIZE);
