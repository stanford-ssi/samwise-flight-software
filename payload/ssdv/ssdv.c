...
static uint16_t const dqt_scales[8] = {5000, 357, 172, 116, 100, 58, 28, 0};

/* Quantisation tables */
static uint8_t const std_dqt0[65] = {
    0x00, 0x10, 0x0C, 0x0C, 0x0E, 0x0C, 0x0A, 0x10, 0x0E, 0x0E, 0x0E,
    0x12, 0x12, 0x10, 0x14, 0x18, 0x28, 0x1A, 0x18, 0x16, 0x16, 0x18,
    0x32, 0x24, 0x26, 0x1E, 0x28, 0x3A, 0x34, 0x3E, 0x3C, 0x3A, 0x34,
    0x38, 0x38, 0x40, 0x48, 0x5C, 0x4E, 0x40, 0x44, 0x58, 0x46, 0x38,
    0x38, 0x50, 0x6E, 0x52, 0x58, 0x60, 0x62, 0x68, 0x68, 0x68, 0x3E,
    0x4E, 0x72, 0x7A, 0x70, 0x64, 0x78, 0x5C, 0x66, 0x68, 0x64,
};

static uint8_t const std_dqt1[65] = {
    0x01, 0x12, 0x12, 0x12, 0x16, 0x16, 0x16, 0x30, 0x1A, 0x1A, 0x30,
    0x64, 0x42, 0x38, 0x42, 0x64, 0x64, 0x64, 0x64, 0x64, 0x64, 0x64,
...
static void load_standard_dqt(uint8_t *dst, const uint8_t *table,
                              uint8_t quality)
{
    int i;
...
static void *sload_standard_dqt(ssdv_t *s, const uint8_t *table,
                                uint8_t quality)
{
    uint8_t *r;

    /* DQT is 65 bytes long, ensure there is space */
    if (s->stbl_len + 65 > TBL_LEN + HBUFF_LEN)
        return (NULL);
...
static void *dload_standard_dqt(ssdv_t *s, const uint8_t *table,
                                uint8_t quality)
{
    uint8_t *r;

    /* DQT is 65 bytes long, ensure there is space */
    if (s->dtbl_len + 65 > TBL_LEN + HBUFF_LEN)
        return (NULL);
...
{
    void *r;
    if (s->stbl_len + n > TBL_LEN + HBUFF_LEN)
        return (NULL);
...
static inline char jpeg_dht_lookup_symbol(ssdv_t *s, uint8_t symbol,
                                          uint16_t *bits, uint8_t *width)
{
    uint16_t code = 0;
    uint8_t cw, n;
    uint8_t *dht, *ss;
...
static char ssdv_outbits_sync(ssdv_t *s)
{
    ...
    if (b)
        return (ssdv_outbits(s, 0xFF, 8 - b));
    return (SSDV_OK);
}
...
static char ssdv_out_jpeg_int(ssdv_t *s, uint8_t rle, int value)
{
    ...
    r = jpeg_dht_lookup_symbol(s, (rle << 4) | (intlen & 0x0F), &huffbits,
                               &hufflen);
...
static char ssdv_process(ssdv_t *s)
{
    if (s->state == S_HUFF)
    {
        ...
        if (s->acpart == 0) /* DC */
        {
            if (symbol == 0x00)
            {
                ...
                if (s->reset_mcu == s->mcu_id &&
                    (s->mcupart == 0 || s->mcupart >= s->ycparts))
                {
    ...
    if (s->acpart >= 64)
    {
        ...
        {
            /* For greyscale input images, pad the 2x1 MCUs with empty colour
             * blocks */
        ...
        if (s->mcupart == s->ycparts + 2)
        {
            ...
            if (s->mode == S_ENCODING && s->packet_mcu_id == 0xFFFF)
            {
                ...
                s->packet_mcu_offset =
                    s->pkt_size_payload - s->out_len + ((s->outlen + 7) / 8);
            }
...
static void ssdv_set_packet_conf(ssdv_t *s)
{
    ...
    switch (s->type)
    {
        case SSDV_TYPE_NORMAL:
            s->pkt_size_payload = s->pkt_size - SSDV_PKT_SIZE_HEADER -
                                  SSDV_PKT_SIZE_CRC - SSDV_PKT_SIZE_RSCODES;
            s->pkt_size_crcdata =
                SSDV_PKT_SIZE_HEADER + s->pkt_size_payload - 1;
            break;
...
static char ssdv_have_marker_data(ssdv_t *s)
{
    ...
    switch (s->marker)
    {
        case J_SOF0:
            ...
            {
                fprintf(stderr,
                        "Error: The image must have a precision of 8\n");
                return (SSDV_ERROR);
            }
            ...
            {
                fprintf(stderr,
                        "Error: The image must have 1 or 3 components\n");
                return (SSDV_ERROR);
            }
            ...
            {
                fprintf(stderr, "Error: The image is too big. Maximum "
                                "resolution is 4080x4080\n");
                return (SSDV_ERROR);
            }
            ...
            if ((s->width & 0x0F) || (s->height & 0x0F))
            {
                fprintf(
                    stderr,
                    "Error: The image dimensions must be a multiple of 16\n");
                return (SSDV_ERROR);
            }
            ...
            for (i = 0; i < d[5]; i++)
            {
                ...
                fprintf(stderr,
                        "DQT table for component %i: %02X, Sampling factor: "
                        "%ix%i\n",
                        dq[0], dq[2], dq[1] & 0x0F, dq[1] >> 4);

                /* The first (Y) component must have a factor of 2x2,2x1,1x2 or
                 * 1x1 */
                if (i == 0)
                {
                    switch (dq[1])
                    {
                        ...
                        case 0x11:
                            ...
                            break;
                        default:
                            fprintf(stderr, "Error: Component 1 sampling "
                ...
                {
                    fprintf(stderr,
                            "Error: Component %i sampling factor must be 1x1\n",
                            dq[0]);
                    return (SSDV_ERROR);
                }
            }
            ...
            {
                fprintf(stderr,
                        "Error: Maximum number of MCU blocks is 65535\n");
                return (SSDV_ERROR);
            }
        ...
        case J_SOS:
            ...
            {
                fprintf(stderr,
                        "Error: The image is missing one or more DQT tables\n");
                return (SSDV_ERROR);
            }
            ...
            {
                fprintf(stderr,
                        "Error: The image is missing one or more DHT tables\n");
                return (SSDV_ERROR);
            }
                   ...
                   int8_t quality, int pkt_size)
{
    ...
    if (pkt_size > SSDV_PKT_SIZE ||
        pkt_size - SSDV_PKT_SIZE_HEADER - SSDV_PKT_SIZE_CRC -
                (type == SSDV_TYPE_NORMAL ? SSDV_PKT_SIZE_RSCODES : 0) <
            2)
    {
        ...
        switch (s->state)
        {
            ...
            case S_INT:
                ...
                if (r == SSDV_BUFFER_FULL || r == SSDV_EOI)
                {
                    ...
                    s->out[11] = 0x00;
                    s->out[11] |= ((s->quality - 4) & 7)
                                  << 3; /* Quality level */
                    s->out[11] |= (r == SSDV_EOI ? 1 : 0)
                                  << 2;               /* EOI flag (1 bit) */
                    ...
                    if (s->out_len > 0)
                        ssdv_memset_prng(s->outp, s->out_len);
                    ...
                    {
                        encode_rs_8(&s->out[1], &s->out[i],
                                    SSDV_PKT_SIZE - s->pkt_size);
                    }
...
static void ssdv_write_marker(ssdv_t *s, uint16_t id, uint16_t length,
                              const uint8_t *data)
{
...
char ssdv_dec_feed(ssdv_t *s, uint8_t *packet)
{
    ...
    if (s->packet_id == 0)
    {
        ...
        s->callsign = (packet[2] << 24) | (packet[3] << 16) | (packet[4] << 8) |
                      packet[5];
        s->image_id = packet[6];
        s->width = packet[9] << 4;
...
char ssdv_dec_is_packet(uint8_t *packet, int pkt_size, int *errors)
{
    ...
    if (pkt[1] == 0x66 + SSDV_TYPE_NOFEC)
    {
        ...
        if (x == (pkt[i + 3] | (pkt[i + 2] << 8) | (pkt[i + 1] << 16) |
                  (pkt[i] << 24)))
        {
    ...
    if (type == SSDV_TYPE_INVALID)
    {
        ...
        if (i < 0)
            return (-1); /* Reed-solomon decoder failed */
        if (errors)
            *errors = i;
    ...
    if (p.mcu_id != 0xFFFF)
    {
        if (p.mcu_id >= p.mcu_count)
            return (-1);
        if (p.mcu_offset >= pkt_size_payload)
            return (-1);
    }
...
void ssdv_dec_header(ssdv_packet_info_t *info, uint8_t *packet)
{
    ...
    if (info->mcu_mode == 1 || info->mcu_mode == 2)
        info->mcu_count *= 2;
    else if (info->mcu_mode == 3)
        info->mcu_count *= 4;
}
