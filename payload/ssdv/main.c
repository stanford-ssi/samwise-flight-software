...
void exit_usage()
{
    fprintf(
        stderr,
        "\n"
        "Usage: ssdv [-e|-d] [-n] [-t <percentage>] [-c <callsign>] [-i <id>] "
        "[-q <level>] [-l <length>] [<in file>] [<out file>]\n"
        "\n"
        ...
        "Packet Length\n"
        "\n"
        "The packet length must be specified for both encoding and decoding if "
        "not\n"
...
int main(int argc, char *argv[])
{
    ...
    while ((c = getopt(argc, argv, "ednc:i:q:l:t:v")) != -1)
    {
        switch (c)
        {
            ...
            case 'c':
                ...
                {
                    fprintf(stderr,
                            "Warning: callsign cropped to 6 characters.\n");
                }
    ...
    switch (encode)
    {
        case 0: /* Decode */
            ...
            while (fread(pkt, pkt_length, 1, fin) > 0)
            {
                ...
                if (verbose)
                {
                    ...
                    fprintf(stderr,
                            "Decoded image packet. Callsign: \"%s\", Image ID: "
                            "%d, Resolution: %dx%d, Packet ID: %d (%d errors "
                            "corrected)\n"
                            ">> Type: %d, Quality: %d, EOI: %d, MCU Mode: %d, "
                            "MCU Offset: %d, MCU ID: %d/%d\n",
                            p.callsign_s, p.image_id, p.width, p.height,
                            p.packet_id, errors, p.type, p.quality, p.eoi,
                            p.mcu_mode, p.mcu_offset, p.mcu_id, p.mcu_count);
                }
        ...
        case 1: /* Encode */
            ...
            if (ssdv_enc_init(&ssdv, type, callsign, image_id, quality,
                              pkt_length) != SSDV_OK)
            {
                return (-1);
            }
