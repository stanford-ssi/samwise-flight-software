...
rfm9x = adafruit_rfm9x.RFM9x(spi, cs, reset, 438.1, crc=False)
...
class PacketBuilder:
    ...
    def create_packet(self, dst, src, flags, seq, data):
        if len(data) > PACKET_MAX_DATA_SIZE:
            raise ValueError(f"Data too large: {len(data)} bytes (max {PACKET_MAX_DATA_SIZE})")
        ...
        # Add data
        packet += data
        packet_with_padding = packet + bytes([0x00] * (PACKET_MAX_DATA_SIZE - len(data)))
        
        # Add boot count and msg_id
        packet += struct.pack('<II', CURRENT_BOOT_COUNT, self.msg_id)
        packet_with_padding += struct.pack('<II', CURRENT_BOOT_COUNT, self.msg_id)
        
        # Calculate HMAC
        h = hmac.new(PACKET_HMAC_PSK, msg=packet_with_padding, digestmod=sha256)

        # print packet_with_padding, %02x each byte on a new line starting with [DEBUG]
        for i, byte in enumerate(packet_with_padding):
            print(f"[DEBUG] {byte:02x}")

        packet += h.digest()
        
        self.msg_id += 1
        return packet[4:] # First 4 bytes are inserted by the RFM9x library, so we skip them
    ...
    def unpack_packet(self, packet_bytes):
        ...
        # Unpack header
        dst, src, flags, seq, data_len = struct.unpack('BBBBB', packet_bytes[:PACKET_HEADER_SIZE])
...
def main():
    h = hmac.new(PACKET_HMAC_PSK, msg="HELLO", digestmod=sha256)
    test = " ".join(f"{byte:02x}" for byte in h.digest())
    print(f"HMAC: {test}")
    return
    while True:
        led.value = True
