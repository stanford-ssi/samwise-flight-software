...
def test_try_get_packet_returns_none_on_rssi_drop():
    ...
    mock_rfm.receive.return_value = bytes([cfg.DOWNLINK_BEACON, len(payload)]) + payload
...
def test_try_get_packet_returns_beacon_data_on_success():
    ...
    callsign = b"KC3WNY"
    payload = state + stats + adcs + callsign
    mock_rfm.receive.return_value = bytes([len(payload), cfg.DOWNLINK_BEACON]) + payload
