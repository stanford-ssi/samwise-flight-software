...
void rfm9x_set_low_freq_mode(rfm9x_t* r, uint8_t low_freq) {
  uint8_t reg = rfm9x_get8(r, _RH_RF95_REG_01_OP_MODE);
  if(low_freq) reg = bit_set(reg, 3);
  else reg = bit_clr(reg, 3);
  rfm9x_put8(r, _RH_RF95_REG_01_OP_MODE, reg);
}
...
void rfm9x_set_lora(rfm9x_t* r, uint8_t lora) {
  uint8_t reg = rfm9x_get8(r, _RH_RF95_REG_01_OP_MODE);
  if(lora) reg = bit_set(reg, 7);
  else reg = bit_clr(reg, 7);
  rfm9x_put8(r, _RH_RF95_REG_01_OP_MODE, reg);
}
...
void rfm9x_set_coding_rate(rfm9x_t* r, uint8_t v) {
  ...
  if(v > 5) denominator = v;
  if(v > 8) denominator = 8;
...
void rfm9x_set_spreading_factor(rfm9x_t* r, uint8_t v) {
  ...
  // We skip all the DETECTION_OPTIMIZE and DETECTION_THRESHOLD stuff because it
  // isn't relevant to the RFM9X family.
...
static uint32_t bw_bins[BW_BIN_COUNT + 1] = {7800, 10400, 15600, 20800, 31250, 41700, 62500, 125000, 250000, 0};
...
uint32_t rfm9x_get_bandwidth(rfm9x_t* r) {
  ...
  if(c >= BW_BIN_COUNT) return 500000;
  else return bw_bins[c];
}


/*
 * Set the TX power. If chip is high power, valid values are [5, 23], otherwise [-1, 14]
 */
...
/*
 * Get the TX power. If chip is high power, valid values are [5, 23], otherwise [-1, 14]
 */
...
void rfm9x_init(rfm9x_t* r) {
  ...
  rfm9x_set_bandwidth(r, 125000); /* Configure 125000 to match Radiohead, see SX1276 errata note 2.3 */
  ...
  rfm9x_set_spreading_factor(r, 7); /* Configure to 7 to match Radiohead library */
...
uint8_t rfm9x_await_rx(rfm9x_t* r) {
  rfm9x_listen(r);
  while (!rfm9x_rx_done(r)); // spin until RX done
  return 1;
}
...
uint8_t rfm9x_receive(
    rfm9x_t* r,
    char* packet,
    uint8_t node,
    uint8_t keep_listening,
    uint8_t with_ack
) {
  uint8_t n_bytes = 0;
  ...
  if (rfm9x_is_crc_enabled(r) && rfm9x_crc_error(r)) {
    ...
    if(r->debug) rfm9x_print_packet("[rfm9x] Received packet", packet, fifo_length);
    ...
    if (
        (with_ack || (packet[3] & _SAP_FLAGS_ACK_REQUEST)) &&
...
uint8_t rfm9x_send(
  ...
  // check that length of data is greater than 0 and less than or equal to PAYLOAD_SIZE using an assert
  assert(l > 0 && l <= PAYLOAD_SIZE);

  // set mode to standby
  rfm9x_set_mode(r, STANDBY_MODE); // Stop receiving to clear FIFO and keep it clear
  ...
  if(r->debug) rfm9x_print_packet("[rfm9x] Transmitting packet", payload, payloadLength);
...
uint8_t rfm9x_send_ack(
  ...
  while(!acked && retries < max_retries) {
    rfm9x_send(
      ...
      data,
      l,
      0, /* Don't keep listening, we do that */
      destination,
      node,
      r->seq,
      flags
    ...
    if(destination == _RH_BROADCAST_ADDRESS) {
      ...
      if(l > 0) { /* Received something */
        if(ack_buffer[3] & _RH_FLAGS_ACK) { /* Was an ACK */
