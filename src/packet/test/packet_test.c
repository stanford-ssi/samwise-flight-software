...
void test_packet_structure_layout()
{
    ...
    assert(offsetof(packet_t, hmac) == sizeof(packet_t) - TC_SHA256_DIGEST_SIZE);
...
void test_packet_data_size()
{
    ...
    packet_t packet;
    size_t expected_data_size = 255 - 5 - 8 - 32; // total - header - footer_no_hmac - hmac
...
int main()
{
    ...
    printf("Note: HMAC authentication tests run with PACKET_HMAC_PSK defined\n");
    #else
    printf("Note: HMAC authentication disabled (PACKET_HMAC_PSK not defined)\n");
    #endif
