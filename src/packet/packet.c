...
static uint32_t last_seen_boot_count = 0;
...
bool is_packet_authenticated(packet_t *packet, uint32_t current_boot_count)
{
    ...
    if (packet->boot_count < current_boot_count) {
        LOG_ERROR("Replay detected: packet boot_count %u < current %u", packet->boot_count, current_boot_count);
        return false;
    }
    if (packet->boot_count == current_boot_count && packet->msg_id <= last_seen_msg_id) {
        LOG_ERROR("Replay detected: packet msg_id %u <= last seen %u for boot_count %u", packet->msg_id, last_seen_msg_id, packet->boot_count);
        return false;
    }

    struct tc_hmac_state_struct hmac;
    tc_hmac_set_key(&hmac, (const uint8_t *)PACKET_HMAC_PSK, PACKET_HMAC_PSK_LEN);
    tc_hmac_init(&hmac);
    ...
    bool authenticated = _compare(out_hmac, packet->hmac, TC_SHA256_DIGEST_SIZE) == 0;
    if (authenticated) {
        // Update last seen msg_id if authenticated and not a replay
        if (packet->boot_count > last_seen_boot_count) {
            last_seen_boot_count = packet->boot_count;
            last_seen_msg_id = packet->msg_id;
        } else if (packet->boot_count == last_seen_boot_count && packet->msg_id > last_seen_msg_id) {
            last_seen_msg_id = packet->msg_id;
        }
    }
