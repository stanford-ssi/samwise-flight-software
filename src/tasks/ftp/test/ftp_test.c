...
{
    lfs_ssize_t lfs_err;
    filesys_error_t code = filesys_reformat_initialize(slate, &lfs_err);
    if (code < 0)
    {
...
int ftp_test_config_constants(void)
{
    ...
    TEST_ASSERT(FTP_NUM_PACKETS_PER_CYCLE <= (8 * FTP_PACKET_TRACKER_SIZE),
...
int ftp_test_send_result_packet_with_file(void)
{
    ...
    filesys_error_t code = filesys_start_file_write(
        &test_slate, fname, file_size, crc, &lfs_err, &blocks_left);
    ...
    FTP_PACKET_TRACKER_T received;
    ftp_tracker_clear(&received); // No packets received yet
    ...
    // Clean up
    filesys_cancel_file_write(&test_slate, &lfs_err);
...
int ftp_test_format_filesystem_command(void)
{
    ...
    filesys_error_t code = filesys_start_file_write(&test_slate, fname, 64, 0x0,
                                                    &lfs_err, &blocks_left);
    TEST_ASSERT(code == FILESYS_OK, "start_file_write should succeed");

    uint8_t buffer[64];
    memset(buffer, 0xAA, sizeof(buffer));
    code = filesys_write_data_to_buffer(&test_slate, buffer, 64, 0, &lfs_err);
    TEST_ASSERT(code == FILESYS_OK, "write_data_to_buffer should succeed");

    filesys_write_buffer_to_mram(&test_slate, 64, &lfs_err);
    filesys_cancel_file_write(&test_slate, &lfs_err);

    // Now format the filesystem
    filesys_error_t result = filesys_reformat_initialize(&test_slate, &lfs_err);
    TEST_ASSERT(result == FILESYS_OK, "filesys_reformat should succeed");

    // Verify file is gone
    lfs_file_t file;
    int err = lfs_file_open(filesys_get_lfs(), &file, fname, LFS_O_RDONLY);
...
int ftp_test_start_file_write_command_success(void)
{
    ...
    filesys_error_t code = filesys_start_file_write(
        &test_slate, cmd_data.fname_str, cmd_data.file_len, cmd_data.file_crc,
        &lfs_err, &blocks_left);

    TEST_ASSERT(code == FILESYS_OK, "filesys_start_file_write should succeed");
    TEST_ASSERT(test_slate.filesys_is_writing_file,
...
int ftp_test_start_file_write_command_already_writing(void)
{
    ...
    filesys_error_t code = filesys_start_file_write(
        &test_slate, fname1, 512, 0x11111111, &lfs_err, &blocks_left);
...
int ftp_test_start_file_write_command_insufficient_space(void)
{
    ...
    filesys_error_t code = filesys_start_file_write(
        &test_slate, fname, huge_size, 0x0, &lfs_err, &blocks_left);

    TEST_ASSERT(code == FILESYS_ERR_NOT_ENOUGH_SPACE,
                "Should fail with FILESYS_ERR_NOT_ENOUGH_SPACE");
...
int ftp_test_write_file_data_command_single_packet(void)
{
    ...
    filesys_error_t code = filesys_start_file_write(
        &test_slate, fname, 256, 0x0, &lfs_err, &blocks_left);
    TEST_ASSERT(code == FILESYS_OK, "start_file_write should succeed");
    ...
    code = filesys_write_data_to_buffer(
        &test_slate, write_cmd.data, FTP_DATA_PAYLOAD_SIZE, offset, &lfs_err);
    TEST_ASSERT(code == FILESYS_OK, "write_data_to_buffer should succeed");
...
int ftp_test_write_file_data_command_complete_cycle(void)
{
    ...
    filesys_error_t code = filesys_start_file_write(
        &test_slate, fname, total_size, 0x0, &lfs_err, &blocks_left);
    TEST_ASSERT(code == FILESYS_OK, "start_file_write should succeed");
    ...
    filesys_error_t error = filesys_write_buffer_to_mram(
        &test_slate, FILESYS_BUFFER_SIZE, &lfs_err);
    TEST_ASSERT(error == FILESYS_OK, "Should write full buffer");
    TEST_ASSERT(!test_slate.filesys_buffer_is_dirty,
...
int ftp_test_write_file_data_command_no_file(void)
{
    ...
    filesys_error_t code = filesys_write_data_to_buffer(
        &test_slate, data, FTP_DATA_PAYLOAD_SIZE, 0, &lfs_err);
    TEST_ASSERT(code == FILESYS_ERR_NO_FILE_WRITING,
                "Should fail with FILESYS_ERR_NO_FILE_WRITING");
...
int ftp_test_cancel_file_write_command_success(void)
{
    ...
    filesys_error_t code = filesys_start_file_write(
        &test_slate, fname, 512, 0x0, &lfs_err, &blocks_left);
    TEST_ASSERT(code == FILESYS_OK, "start_file_write should succeed");

    // Write some data and flush to MRAM
    uint8_t data[64];
    memset(data, 0xDD, sizeof(data));
    code = filesys_write_data_to_buffer(&test_slate, data, 64, 0, &lfs_err);
    TEST_ASSERT(code == FILESYS_OK, "write_data_to_buffer should succeed");

    filesys_write_buffer_to_mram(&test_slate, 64, &lfs_err);

    // Cancel file write
    code = filesys_cancel_file_write(&test_slate, &lfs_err);
...
int ftp_test_cancel_file_write_command_no_file(void)
{
    ...
    filesys_error_t code = filesys_cancel_file_write(&test_slate, &lfs_err);
    TEST_ASSERT(code == FILESYS_ERR_NO_FILE_WRITING,
                "Should fail with FILESYS_ERR_NO_FILE_WRITING");
...
int ftp_test_eof_success_correct_crc(void)
{
    ...
    filesys_error_t code = filesys_start_file_write(
        &test_slate, fname, 64, correct_crc, &lfs_err, &blocks_left);
    TEST_ASSERT(code == FILESYS_OK, "start_file_write should succeed");
    ...
    filesys_error_t error =
        filesys_write_buffer_to_mram(&test_slate, 64, &lfs_err);
    TEST_ASSERT(error == FILESYS_OK, "write_buffer_to_mram should succeed");

    // Verify CRC is correct
    filesys_error_t crc_result = filesys_is_crc_correct(&test_slate, &lfs_err);
    TEST_ASSERT(crc_result == FILESYS_OK, "CRC should be correct");

    // Get the computed CRC value for verification
    filesys_error_t crc_err;
    unsigned int computed_crc =
        filesys_compute_crc(&test_slate, &crc_err, &lfs_err);
    TEST_ASSERT(crc_err == FILESYS_OK, "CRC computation should succeed");
    TEST_ASSERT(computed_crc == correct_crc, "Computed CRC should match");

    // Complete file write
    code = filesys_complete_file_write(&test_slate, &lfs_err);
...
int ftp_test_eof_error_crc_mismatch(void)
{
    ...
    filesys_error_t code = filesys_start_file_write(
        &test_slate, fname, 64, wrong_crc, &lfs_err, &blocks_left);
    TEST_ASSERT(code == FILESYS_OK, "start_file_write should succeed");
    ...
    filesys_error_t crc_result = filesys_is_crc_correct(&test_slate, &lfs_err);
    TEST_ASSERT(crc_result != FILESYS_OK, "CRC should be incorrect");

    // Try to complete - should fail with CRC mismatch
    code = filesys_complete_file_write(&test_slate, &lfs_err);
    TEST_ASSERT(
...
int ftp_test_packet_tracker_bitfield(void)
{
    ...
    // Set bits for all packets in a cycle
    ...
    TEST_ASSERT(ftp_tracker_check_mask(&tracker, FTP_NUM_PACKETS_PER_CYCLE),
                "All packet bits should be set");

    // Test individual packet tracking
    ftp_tracker_clear(&tracker);
    ftp_tracker_set_bit(&tracker, 0); // Packet 0 received
    ftp_tracker_set_bit(&tracker, 2); // Packet 2 received
    ftp_tracker_set_bit(&tracker, 4); // Packet 4 received

    TEST_ASSERT(tracker.bytes[0] & (1 << 0), "Packet 0 should be marked");
    TEST_ASSERT(!(tracker.bytes[0] & (1 << 1)),
                "Packet 1 should not be marked");
    TEST_ASSERT(tracker.bytes[0] & (1 << 2), "Packet 2 should be marked");
    TEST_ASSERT(!(tracker.bytes[0] & (1 << 3)),
                "Packet 3 should not be marked");
    TEST_ASSERT(tracker.bytes[0] & (1 << 4), "Packet 4 should be marked");

    // Partial mask check: only 3 bits set but checking 5 should fail
    TEST_ASSERT(!ftp_tracker_check_mask(&tracker, 5),
                "Incomplete cycle should not pass mask check");
...
int ftp_test_multiple_write_cycles(void)
{
    ...
    for (int cycle = 0; cycle < 3; cycle++)
    {
        ...
        for (int pkt = 0; pkt < FTP_NUM_PACKETS_PER_CYCLE; pkt++)
        {
            ...
            code = filesys_write_data_to_buffer(
                &test_slate, data, FTP_DATA_PAYLOAD_SIZE, offset, &lfs_err);
            TEST_ASSERT(code == FILESYS_OK,
                        "write_data_to_buffer should succeed");
        }
