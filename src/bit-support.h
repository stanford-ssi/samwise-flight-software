...
bit_clr(uint32_t x, uint32_t bit) {
    assert(bit<32);
    return x & ~(1<<bit);
}
...
static inline uint32_t 
bit_set(uint32_t x, uint32_t bit) {
    assert(bit<32);
    return x | (1<<bit);
}

static inline uint32_t 
bit_not(uint32_t x, uint32_t bit) {
    assert(bit<32);
    return x ^ (1<<bit);
}
...
// is x[bit] == 1?
static inline uint32_t 
bit_is_on(uint32_t x, uint32_t bit) {
    assert(bit<32);
    return (x >> bit) & 1;
}
static inline uint32_t 
bit_is_off(uint32_t x, uint32_t bit) {
...
static inline uint32_t 
bits_get(uint32_t x, uint32_t lb, uint32_t ub) {
    assert(lb <= ub);
    assert(ub < 32);
...
static inline uint32_t 
bits_clr(uint32_t x, uint32_t lb, uint32_t ub) {
    assert(lb <= ub);
    assert(ub < 32);
...
static inline uint32_t 
bits_set(uint32_t x, uint32_t lb, uint32_t ub, uint32_t v) {
    assert(lb <= ub);
    assert(ub < 32);
...
// bits [lb:ub] == <val?
static inline uint32_t 
bits_eq(uint32_t x, uint32_t lb, uint32_t ub, uint32_t val) {
    assert(lb <= ub);
    assert(ub < 32);
