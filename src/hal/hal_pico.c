...
static void hal_pico_gpio_set_function(hal_pin_t pin,
...
static void hal_pico_gpio_set_irq_enabled_with_callback(
    hal_pin_t pin, uint32_t events, bool enabled, hal_irq_callback_t callback)
{
    gpio_set_irq_enabled_with_callback(pin, events, enabled,
...
static void hal_pico_spi_set_format(hal_spi_t spi, uint8_t data_bits,
                                    uint8_t cpol, uint8_t cpha, uint8_t order)
{
    spi_set_format((spi_inst_t *)spi, data_bits, (spi_cpol_t)cpol,
                   (spi_cpha_t)cpha, (spi_order_t)order);
}

static int hal_pico_spi_write_blocking(hal_spi_t spi, const uint8_t *src,
                                       size_t len)
{
    return spi_write_blocking((spi_inst_t *)spi, src, len);
}

static int hal_pico_spi_read_blocking(hal_spi_t spi, uint8_t repeated_tx_data,
                                      uint8_t *dst, size_t len)
{
    return spi_read_blocking((spi_inst_t *)spi, repeated_tx_data, dst, len);
}

static int hal_pico_spi_write_read_blocking(hal_spi_t spi, const uint8_t *src,
                                            uint8_t *dst, size_t len)
{
...
static int hal_pico_i2c_write_blocking_until(hal_i2c_t i2c, uint8_t addr,
                                             const uint8_t *src, size_t len,
                                             bool nostop, uint64_t until)
{
    absolute_time_t until_time = from_us_since_boot(until);
    return i2c_write_blocking_until((i2c_inst_t *)i2c, addr, src, len, nostop,
                                    until_time);
}

static int hal_pico_i2c_read_blocking_until(hal_i2c_t i2c, uint8_t addr,
                                            uint8_t *dst, size_t len,
                                            bool nostop, uint64_t until)
{
    absolute_time_t until_time = from_us_since_boot(until);
    return i2c_read_blocking_until((i2c_inst_t *)i2c, addr, dst, len, nostop,
                                   until_time);
}
...
static void hal_pico_uart_set_format(hal_uart_t uart, uint8_t data_bits,
                                     uint8_t stop_bits, uint8_t parity)
{
    uart_set_format((uart_inst_t *)uart, data_bits, stop_bits,
...
static void hal_pico_pwm_set_chan_level(unsigned int slice_num,
                                        unsigned int chan, uint16_t level)
{
...
static void hal_pico_flash_range_program(uint32_t flash_offs,
...
void hal_init(void)
{
    ...
    hal.gpio_set_irq_enabled_with_callback =
        hal_pico_gpio_set_irq_enabled_with_callback;
