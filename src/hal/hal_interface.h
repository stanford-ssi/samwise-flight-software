 ...
 *
 * This HAL provides a unified interface for hardware operations while
 * maintaining zero performance overhead for embedded builds. For embedded
 * targets, HAL calls are implemented as direct function calls to Pico SDK
 * functions. For test builds, HAL calls are routed through mock
 * implementations.
 */
...
typedef struct
{
    ...
    void (*gpio_set_irq_enabled_with_callback)(hal_pin_t pin, uint32_t events,
                                               bool enabled,
                                               hal_irq_callback_t callback);

    // SPI Interface
    unsigned int (*spi_init)(hal_spi_t spi, uint32_t baudrate);
    void (*spi_set_format)(hal_spi_t spi, uint8_t data_bits, uint8_t cpol,
                           uint8_t cpha, uint8_t order);
    int (*spi_write_blocking)(hal_spi_t spi, const uint8_t *src, size_t len);
    int (*spi_read_blocking)(hal_spi_t spi, uint8_t repeated_tx_data,
                             uint8_t *dst, size_t len);
    int (*spi_write_read_blocking)(hal_spi_t spi, const uint8_t *src,
                                   uint8_t *dst, size_t len);

    // I2C Interface
    unsigned int (*i2c_init)(hal_i2c_t i2c, uint32_t baudrate);
    int (*i2c_write_blocking_until)(hal_i2c_t i2c, uint8_t addr,
                                    const uint8_t *src, size_t len, bool nostop,
                                    uint64_t until);
    int (*i2c_read_blocking_until)(hal_i2c_t i2c, uint8_t addr, uint8_t *dst,
                                   size_t len, bool nostop, uint64_t until);

    // UART Interface
    unsigned int (*uart_init)(hal_uart_t uart, uint32_t baudrate);
    void (*uart_set_format)(hal_uart_t uart, uint8_t data_bits,
    ...
    void (*pwm_set_chan_level)(unsigned int slice_num, unsigned int chan,
                               uint16_t level);
