...
uint32_t read_flash_word(uint32_t* flash_offset) {
    ...
    // printf("Trying to read from: %p\n", (uint32_t *)(XIP_NOCACHE_NOALLOC_BASE + flash_offset));
    // uint32_t *addr = (uint32_t *)(XIP_NOCACHE_NOALLOC_BASE + flash_offset);
    return *flash_offset;
}
...
#define PICO_DEFAULT_LED_PIN 25
    ...
    while (1)
    {
        ...
        // Buffer to hold partition table info (36 words should be enough for most cases)
        uint32_t buffer[128];
        
        // Request partition table info and partition locations
        uint32_t flags = PT_INFO_PT_INFO | PT_INFO_PARTITION_LOCATION_AND_FLAGS;
        
        int result = rom_get_partition_table_info(buffer, sizeof(buffer)/sizeof(uint32_t), flags);
        ...
        if (buffer[0] & PT_INFO_PT_INFO) {
            ...
            // For each partition: permissions_and_location, permissions_and_flags (2 words)
            int idx = 2;
            for (uint32_t i = 0; i < num_partitions+1; i++) {
                ...
                uint32_t location_first_sector_block = perms_and_loc & 0x00001fffu;
                uint32_t location_last_sector_block = (perms_and_loc & 0x03ffe000u) >> 13;
                
                // Word 1: permissions_and_flags (additional partition info)
                uint32_t perms_and_flags = buffer[idx++];
                uint32_t perms_secure_read_flags = perms_and_flags & 0x04000000u;
                uint32_t perms_secure_write_flags = perms_and_flags & 0x08000000u;
                uint32_t perms_nonsecure_read_flags = perms_and_flags & 0x10000000u;
                uint32_t perms_nonsecure_write_flags = perms_and_flags & 0x20000000u;
                
                if (i == 0) {
                    printf("Unpartitioned Space:\n", i);
                    printf("  Location: 0x%08x (%u KB from flash start)\n", 
                        location_first_sector_block * 4096, location_first_sector_block * 4);
                    printf("  Size: %u KB\n", 
                        (location_last_sector_block - location_first_sector_block + 1) * 4);
                    
                    // Decode permission bits (exact bit positions depend on implementation)
        ...
        // <--- this finally work!!! --->
        int ret = rom_reboot(BOOT_TYPE_FLASH_UPDATE, 200, XIP_BASE + 0x42000, 0);
        ...
        // rom_reboot(BOOT_TYPE_RAM_IMAGE, 50, 0x42000 /* word-aligned flash start */, 0x10000 /* blocked size */);
